...
const sourceRoot = process.env.FLUXMQ_DOCS_ROOT ?? '/Users/lulu/Work/fluxmq-rs-doc'
const outputFile = path.join(repoRoot, 'src/fluxmqDocs.ts')
const assetOutputDir = path.join(repoRoot, 'public/fluxmq-docs')
...
const preprocessBody = (body) => {
  let normalized = body
    .replace(/<Screenshot\s+name="([^"]+)"\s+alt="([^"]+)"\s*\/>/g, (_, name, alt) => {
      ...
      return `\n![${alt}](/fluxmq-docs/img/screenshots/${name}.png)\n`
    })
    .replace(/<video[\s\S]*?src="([^"]+)"[\s\S]*?\/>/g, (_, src) => `\n@@VIDEO|${rewriteAssetPath(src)}|FluxMQ demo video@@\n`)
...
const buildCatalog = (locale, docsDir) => {
  ...
  return {
    locale,
    productName: 'FluxMQ',
    title: locale === 'zh' ? 'FluxMQ 文档中心' : 'FluxMQ Documentation',
    eyebrow: 'FluxMQ Docs',
    body:
      locale === 'zh'
        ? '从 fluxmq-rs-doc 迁移而来的 FluxMQ 产品、安装、功能、API、规则引擎、数据源和运维手册，官网统一以 1.0 版本维护。'
        : 'FluxMQ product, installation, feature, API, rule engine, data source, and operations documentation migrated from fluxmq-rs-doc and maintained here as version 1.0.',
    searchPlaceholder: locale === 'zh' ? '浏览 FluxMQ 文档、规则、API' : 'Browse FluxMQ docs, rules and APIs',
    sidebarLabel: locale === 'zh' ? 'FluxMQ 文档目录' : 'FluxMQ documentation navigation',
    ...
    versions: [
      {
        ...
        note:
          locale === 'zh'
            ? '当前官网文档版本，内容来源于 fluxmq-rs-doc，并按官网文档系统重新排版。'
            : 'Current website documentation version, migrated from fluxmq-rs-doc and restyled for the website docs system.',
        command: 'FLUXMQ_CONFIG_FILE=config.toml cargo run -p fluxmqd',
      },
    ],
...
const output = `// Generated by scripts/import-fluxmq-docs.mjs from ${sourceRoot}\n\nexport type FluxMQDocInlineText = string\n\nexport type FluxMQDocBlock =\n  | { type: 'heading'; id: string; level: number; text: string }\n  | { type: 'paragraph'; text: string }\n  | { type: 'list'; ordered: boolean; items: string[] }\n  | { type: 'code'; language: string; code: string }\n  | { type: 'table'; headers: string[]; rows: string[][] }\n  | { type: 'image'; src: string; alt: string }\n  | { type: 'video'; src: string; title: string }\n\nexport type FluxMQDocHeading = { id: string; level: number; text: string }\nexport type FluxMQDocDocument = {\n  id: string\n  title: string\n  summary: string\n  sourcePath: string\n  headings: FluxMQDocHeading[]\n  blocks: FluxMQDocBlock[]\n}\nexport type FluxMQDocNavEntry =\n  | { type: 'category'; label: string; depth: number }\n  | { type: 'doc'; id: string; label: string; depth: number }\nexport type FluxMQDocGroup = { title: string; entries: FluxMQDocNavEntry[] }\nexport type FluxMQDocVersion = { id: string; label: string; status: string; date: string; note: string; command: string }\nexport type FluxMQDocsCatalog = {\n  locale: 'zh' | 'en'\n  productName: string\n  title: string\n  eyebrow: string\n  body: string\n  searchPlaceholder: string\n  sidebarLabel: string\n  tocLabel: string\n  versionLabel: string\n  versionStatusLabel: string\n  commandLabel: string\n  defaultDocumentId: string\n  versions: FluxMQDocVersion[]\n  groups: FluxMQDocGroup[]\n  documents: Record<string, FluxMQDocDocument>\n}\n\nexport const fluxmqDocs: Record<'zh' | 'en', FluxMQDocsCatalog> = ${JSON.stringify(generated, null, 2)}\n`
